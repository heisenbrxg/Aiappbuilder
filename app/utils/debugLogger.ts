...
// Starsky doesn't have PROVIDER_LIST/DEFAULT_MODEL in constants
...
export async function downloadDebugLog(filename?: string): Promise<void> {
  try {
    ...
    link.href = url;
    link.download = filename || `monzed-debug-${new Date().toISOString().split('T')[0]}.txt`;
