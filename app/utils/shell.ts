...
export async function newShellProcess(webcontainer: WebContainer, terminal: ITerminal) {
  ...
  // monzed.diy optimization: Batch terminal writes using RAF for 60fps performance
  ...
  output.pipeTo(
    new WritableStream({
      write(data) {
        ...
        // Batch writes with requestAnimationFrame (monzed.diy pattern)
        pendingData += data;
        
        if (rafId === null) {
          rafId = requestAnimationFrame(flushTerminalData);
        }
      },
    }),
  );
  ...
  // Auto-respawn shell on unexpected exit
  process.exit.then((code: number) => {
    try {
      terminal.write(`\r\n[Shell exited (${code}); reconnecting…]\r\n`);
    } catch {}
    newShellProcess(webcontainer, terminal).catch(console.error);
  });
...
export class BoltShell {
  ...
  async init(webcontainer: WebContainer, terminal: ITerminal) {
    ...
    // Auto-respawn monzed shell on crash
    process.exit.then(async (code: number) => {
      try {
        terminal.write(`\r\n[Shell exited (${code}); reconnecting…]\r\n`);
      } catch {}
      try {
        const { process: newProc, commandStream: cs, expoUrlStream: es } = await this.newBoltShellProcess(webcontainer, terminal);
        this.#process = newProc;
        this.#outputStream = cs.getReader();
        this._watchExpoUrlInBackground(es);
      } catch (err) {
        console.error('Failed to respawn monzed shell', err);
      }
    });
