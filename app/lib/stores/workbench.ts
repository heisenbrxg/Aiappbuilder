...
// JSZip loaded dynamically to avoid circular dependency issues
import fileSaver from 'file-saver';
import { WPThemeBuilder, type WPThemeOptions } from '~/lib/services/wpThemeBuilder';
...
export type WorkbenchViewType = 'code' | 'diff' | 'preview' | 'mcp';
...
export class WorkbenchStore {
  ...
  /**
   * Track commands (type + content) that have already been executed in the
   * current session when replaying historical messages so we don't run them
   * multiple times, but we still run the essential first occurrence (e.g.
   * `npm install`, `npm run dev`).
   */
  #executedCommands: Set<string> = new Set();
  #restorationMessageIds: Set<string> = new Set();
  ...
  #snapshotSaveTimeout: NodeJS.Timeout | null = null;
  
  async #saveSnapshotAfterAction() {
    // Clear any existing timeout to debounce rapid calls
    if (this.#snapshotSaveTimeout) {
      clearTimeout(this.#snapshotSaveTimeout);
    }

    // Debounce snapshot saving to prevent infinite loops
    this.#snapshotSaveTimeout = setTimeout(async () => {
      try {
        const userFiles = this.files.get();
        const user = supabaseConnection.get().user;
        const currentChatId = chatId.get();

        // Validate user authentication before attempting to save
        if (!user || !user.id) {
          return;
        }
        ...
        await new SupabaseDB().setSnapshot(user.id, currentChatId, userFiles);
      } catch (error) {
        console.error('Failed to save snapshot after action:', error);
  ...
  constructor() {
    const canUseBroadcastChannel = typeof window !== 'undefined' && typeof BroadcastChannel !== 'undefined';

    this.#previewsStore = canUseBroadcastChannel
      ? new PreviewsStore(webcontainer)
      : ( {
          // Minimal stub that satisfies the interface used within this store.
          previews: atom([]),
        } as unknown as PreviewsStore );
  ...
  setShowWorkbench(show: boolean) {
    ...
    // UI state synchronization is handled by syncWorkbenchState() in uiState.ts
  ...
  addArtifact({ messageId, title, id, type }: ArtifactCallbackData) {
    ...
    // Track restoration messages separately
    if (id === 'restored-project-setup') {
      this.#restorationMessageIds.add(messageId);
    }
  ...
  private _markActionExecuted(data: ActionCallbackData) {
    const { artifactId } = data;
    const artifact = this.#getArtifact(artifactId);
    if (!artifact) {
      return;
  ...
  async _runAction(data: ActionCallbackData, isStreaming: boolean = false) {
    ...
    // Ensure the Workbench is visible when code or preview actions start executing
    if (!this.showWorkbench.get() && (data.action.type === 'file' || data.action.type === 'start')) {
    ...
    if (data.action.type === 'file') {
      ...
      // Keep current view instead of forcing code view
      // Users can manually switch if they want to see the code
  ...
  /**
   * Export the current project as a WordPress theme ZIP (ready for upload).
   * @param options Optional theme metadata overrides.
   */
  async exportWordPressTheme(options: Partial<WPThemeOptions> = {}) {
    const files = this.files.get();
    ...
    const themeName = (
      options.themeName ??
      (description.value ?? 'Digimetrix_theme')
    )
      .toLocaleLowerCase()
      .split(' ')
      .join('_');

    const wpThemeFiles = await builder.buildTheme(files, {
      themeName: themeName,
      version: '1.0.0',
      ...options,
    } as WPThemeOptions);
    ...
    for (const [filePath, content] of Object.entries(wpThemeFiles)) {
