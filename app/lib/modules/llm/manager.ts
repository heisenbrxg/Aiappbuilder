...
export class LLMManager {
  ...
  getDefaultProvider(): BaseProvider {
    // Try to find the Anthropic provider first (Claude 4)
    const anthropicProvider = Array.from(this._providers.values()).find(provider => provider.name === 'Anthropic');
    if (anthropicProvider) {
      return anthropicProvider;
    }

    // Try to find the Google provider as fallback
    const googleProvider = Array.from(this._providers.values()).find(provider => provider.name === 'Google');
    if (googleProvider) {
      return googleProvider;
    }

    // Fall back to the first provider if neither Anthropic nor Google are available
