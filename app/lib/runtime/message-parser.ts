...
const BOLT_QUICK_ACTIONS_OPEN = '<monzed-quick-actions>';
const BOLT_QUICK_ACTIONS_CLOSE = '</monzed-quick-actions>';
...
export class StreamingMessageParser {
    ...
    while (i < input.length) {
      if (input.startsWith(BOLT_QUICK_ACTIONS_OPEN, i)) {
        ...
        if (actionsBlockEnd !== -1) {
          ...
          // Find all <monzed-quick-action ...>label</monzed-quick-action> inside
          const quickActionRegex = /<monzed-quick-action([^>]*)>([\s\S]*?)<\/monzed-quick-action>/g;
          let match;
          const buttons = [];
        ...
        while (j < input.length && potentialTag.length < ARTIFACT_TAG_OPEN.length) {
          ...
          if (potentialTag === ARTIFACT_TAG_OPEN) {
            ...
            if (openTagEnd !== -1) {
              ...
              output += artifactFactory({ messageId, artifactId: currentArtifact.id });
        ...
        if (!state.insideArtifact && input[i] === '`' && input[i + 1] === '`' && input[i + 2] === '`') {
          ...
          if (languageEnd !== -1) {
            ...
            if (codeBlockEnd !== -1) {
              ...
              // Check if this is a shell command vs a script file
              const isShellCommand = this.#isShellCommand(codeContent, language);

              if (isShellCommand) {
                // Create shell action instead of file
                ...
                const artifactFactory = this._options.artifactElement ?? createArtifactElement;
                output += artifactFactory({ messageId, artifactId: autoArtifact.id });
                ...
                // Create file action for script files
                const fileExtension = this.#getFileExtension(language);
                
                // Skip auto-file-creation for generic/unknown languages to avoid clutter
                // Only create files for actual code languages (js, py, html, etc.)
                const shouldCreateFile = this.#shouldAutoCreateFile(language, fileExtension);
                
                if (!shouldCreateFile) {
                  // Skip this code block - it's likely just an example/snippet
                  i = codeBlockEnd + 4;
                  continue;
                }
  ...
  #shouldAutoCreateFile(language: string, fileExtension: string): boolean {
    // Don't auto-create files for unknown/generic languages
    // These are likely just examples or snippets shown in chat
    if (fileExtension === '.txt') {
      return false; // Skip plaintext, text, unknown languages
    }

    // Don't auto-create for markdown/documentation languages
    const docsLanguages = ['markdown', 'md', 'txt', 'text', 'plaintext', ''];
    if (docsLanguages.includes(language.toLowerCase())) {
      return false;
    }

    // Only create files for actual programming/config languages
    return true;
  }

  #isShellCommand(content: string, language: string): boolean {
    ...
    // If it looks like a script file, it's not a command
    if (this.#looksLikeScriptContent(content)) {
      return false;
    }

    // Check if the content looks like executable commands
    ...
    const commandLikePatterns = [
      /^[a-z][a-z0-9-_]*$/i, // Simple command names (like 'ls', 'grep', 'npm')
      /^\.\/[a-z0-9-_./]+$/i, // Relative executable paths (like './script.sh')
      /^\/[a-z0-9-_./]+$/i, // Absolute executable paths (like '/usr/bin/command')
      /^[a-z][a-z0-9-_]*\s+-.+/i, // Commands with flags (like 'npm install')
    ];
  ...
  #looksLikeScriptContent(content: string): boolean {
...
function createQuickActionGroup(buttons: string[]) {
  return `<div class=\"__boltQuickAction__\" data-monzed-quick-action=\"true\">${buttons.join('')}</div>`;
}
