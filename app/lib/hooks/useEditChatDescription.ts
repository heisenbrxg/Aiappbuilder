...
export function useEditChatDescription({
  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent) => {
      ...
      try {
        console.log('Updating chat description:', {
          user: user?.id,
          urlId: customChatId || window.location.pathname.split('/').pop(),
          description: currentDescription
        });

        if (user) {
          // Use Supabase when authenticated
          const urlId = customChatId || window.location.pathname.split('/').pop();
          
          if (!urlId) {
            toast.error('Chat URL ID is not available');
            return;
          }
          
          await supabaseDB.updateChatDescription(user.id, urlId, currentDescription);
        ...
        // Show success message only once after all operations are complete
    ...
    [currentDescription, db, chatId, initialDescription, customChatId, user, syncWithGlobalStore, isValidDescription, toggleEditMode, supabaseDB],
  );
