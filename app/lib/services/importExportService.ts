...
export class ImportExportService {
   ...
   * @param db The IndexedDB database instance (optional, for fallback)
   * @param userId The user ID for Supabase (optional, for authenticated users)
   * @returns A promise that resolves to the export data
   */
  static async exportAllChats(db?: IDBDatabase, userId?: string): Promise<{ chats: any[]; exportDate: string }> {
    try {
      let chats: any[];
      ...
      if (userId) {
        ...
        const supabaseChats = await supabaseDB.getAllChats(userId);
        ...
        chats = supabaseChats.map((chat) => ({
          ...
          urlId: chat.url_id || null,
          metadata: chat.metadata || null,
        }));
      } else if (db) {
        // Fallback to IndexedDB when not authenticated
        const indexedDBChats = await getAllChats(db);

        chats = indexedDBChats.map((chat) => ({
          id: chat.id,
        ...
        throw new Error('No database available for export');
      }

      console.log(`Successfully prepared ${chats.length} chats for export`);
   ...
   * @param userId The user ID for Supabase (optional, for authenticated users)
   */
  static async deleteAllChats(db?: IDBDatabase, userId?: string): Promise<void> {
    try {
      if (userId) {
        // Delete from Supabase when authenticated
        const supabaseDB = new SupabaseDB();
        const chats = await supabaseDB.getAllChats(userId);
        const deletePromises = chats.map((chat) => supabaseDB.deleteChat(userId, chat.url_id));
        await Promise.all(deletePromises);
      } else if (db) {
        // Fallback to IndexedDB when not authenticated
        ...
        throw new Error('No database available for deletion');
      }
    } catch (error) {
