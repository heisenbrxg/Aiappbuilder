...
/**
 * Generate a unique URL ID for new chats
 * Uses UUID v4 format for guaranteed uniqueness
 */
export async function generateUniqueUrlId(db: IDBDatabase): Promise<string> {
  const { generateChatUrlId } = await import('~/utils/fileUtils');

  // UUID v4 has extremely low collision probability, but we'll still check for safety
  const existingIds = await getUrlIds(db);
  let newId = generateChatUrlId();

  // In the extremely unlikely case of a collision, generate a new one
  while (existingIds.includes(newId)) {
    newId = generateChatUrlId();
  }
...
export async function createChatFromMessages(
  ...
  const newUrlId = await generateUniqueUrlId(db); // Generate unique alphanumeric URL ID

  await setMessages(
    db,
    newId,
    messages,
    newUrlId, // Use the new unique urlId
    description,
