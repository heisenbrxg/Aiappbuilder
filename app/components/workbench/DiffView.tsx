...
const FullscreenButton = memo(({ onClick, isFullscreen }: FullscreenButtonProps) => (
  <button
    onClick={onClick}
    className="ml-4 p-1 rounded hover:bg-monzed-elements-background-depth-3 text-monzed-elements-textTertiary hover:text-monzed-elements-textPrimary transition-colors"
...
const FullscreenOverlay = memo(({ isFullscreen, children }: { isFullscreen: boolean; children: React.ReactNode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/50 flex items-center justify-center p-6">
      <div className="w-full h-full max-w-[90vw] max-h-[90vh] bg-monzed-elements-background-depth-2 rounded-lg border border-monzed-elements-borderColor shadow-xl overflow-hidden">
        {children}
      </div>
    </div>
  );
...
const lineNumberStyles =
  'w-9 shrink-0 pl-2 py-1 text-left font-mono text-monzed-elements-textTertiary border-r border-monzed-elements-borderColor bg-monzed-elements-background-depth-1';
const lineContentStyles =
  'px-1 py-1 font-mono whitespace-pre flex-1 group-hover:bg-monzed-elements-background-depth-2 text-monzed-elements-textPrimary';
...
const diffLineStyles = {
  ...
  removed: 'bg-orange-500/10 dark:bg-orange-500/20 border-l-4 border-orange-500',
  unchanged: '',
};

const changeColorStyles = {
  added: 'text-green-700 dark:text-green-500 bg-green-500/10 dark:bg-green-500/20',
  removed: 'text-orange-700 dark:text-orange-500 bg-orange-500/10 dark:bg-orange-500/20',
  unchanged: 'text-monzed-elements-textPrimary',
};

const renderContentWarning = (type: 'binary' | 'error') => (
  <div className="h-full flex items-center justify-center p-4">
    <div className="text-center text-monzed-elements-textTertiary">
      <div className={`i-ph:${type === 'binary' ? 'file-x' : 'warning-circle'} text-4xl text-orange-400 mb-2 mx-auto`} />
...
const NoChangesView = memo(
  ({
    ...
    useEffect(() => {
      const highlightCode = async () => {
        ...
        const highlighted = await Promise.all(
          lines.map(async (line) => {
            try {
              const result = await codeToHtml(line, {
                lang: language,
                theme: theme === 'dark' ? 'dark-plus' : 'light-plus',
              });
              return result.replace(/<\/?pre[^>]*>/g, '').replace(/<\/?code[^>]*>/g, '');
            } catch {
              return line;
            }
          })
      ...
      highlightCode();
    }, [beforeCode, language, theme]);

    return (
      <div className="h-full flex flex-col items-center justify-center p-4">
        <div className="text-center text-monzed-elements-textTertiary">
          <div className="i-ph:files text-4xl text-green-400 mb-2 mx-auto" />
          <p className="font-medium text-monzed-elements-textPrimary">Files are identical</p>
          <p className="text-sm mt-1">Both versions match exactly</p>
        </div>
        <div className="mt-4 w-full max-w-2xl bg-monzed-elements-background-depth-1 rounded-lg border border-monzed-elements-borderColor overflow-hidden">
          <div className="p-2 text-xs font-bold text-monzed-elements-textTertiary border-b border-monzed-elements-borderColor">
            Current Content
          </div>
...
const CodeLine = memo(
  ({
    ...
    useEffect(() => {
      const highlightCode = async () => {
        try {
          if (type === 'unchanged' || !block.charChanges) {
            ...
            setHighlightedContent(result.replace(/<\/?pre[^>]*>/g, '').replace(/<\/?code[^>]*>/g, ''));
          } else {
            // Handle character-level changes
            const parts = await Promise.all(
              block.charChanges!.map(async (change) => {
                const changeClass = changeColorStyles[change.type];
                try {
                  const result = await highlighter.codeToHtml(change.value, {
                    lang: language,
                    theme: theme === 'dark' ? 'dark' : 'light',
                  });
                  const cleaned = result.replace(/<\/?pre[^>]*>/g, '').replace(/<\/?code[^>]*>/g, '');
                  return `<span class="${changeClass}">${cleaned}</span>`;
                } catch {
                  return `<span class="${changeClass}">${change.value}</span>`;
                }
              })
            );
            setHighlightedContent(parts.join(''));
          }
        } catch {
      ...
      highlightCode();
    }, [content, type, block.charChanges, highlighter, language, theme]);

    return (
      <div className="flex group min-w-fit">
        <div className={lineNumberStyles}>{lineNumber + 1}</div>
        <div className={`${lineContentStyles} ${bgColor}`}>
          <span className="mr-2 text-monzed-elements-textTertiary">
            {type === 'added' && <span className="text-green-700 dark:text-green-500">+</span>}
            {type === 'removed' && <span className="text-orange-700 dark:text-orange-500">-</span>}
            {type === 'unchanged' && ' '}
          </span>
          <span dangerouslySetInnerHTML={{ __html: highlightedContent || content }} />
        </div>
      </div>
    );
  },
...
const FileInfo = memo(
  ({
    ...
    return (
      <div className="flex items-center bg-monzed-elements-background-depth-1 p-2 text-sm text-monzed-elements-textPrimary shrink-0">
            ...
            <>
              {showStats && (
                <div className="flex items-center gap-1 text-xs">
                  ...
                  {deletions > 0 && <span className="text-orange-700 dark:text-orange-500">-{deletions}</span>}
                </div>
              )}
              <span className="text-blue-600 dark:text-blue-400">Modified</span>
              <span className="text-monzed-elements-textTertiary text-xs">{new Date().toLocaleTimeString()}</span>
            </>
          ) : (
...
// Create a local adapter-based highlighter with caching
const createCachedHighlighter = () => {
  const cache = new Map<string, string>();
  
  return {
    codeToHtml: async (code: string, options: { lang: string; theme?: string }) => {
      const cacheKey = `${code}-${options.lang}-${options.theme}`;
      ...
      try {
        const result = await codeToHtml(code, {
          lang: options.lang,
          theme: options.theme === 'dark' ? 'dark-plus' : 'light-plus',
        });
...
const InlineDiffComparison = memo(({ beforeCode, afterCode, filename, language }: CodeComparisonProps) => {
  ...
  // Use cached highlighter for better performance
  const highlighter = useMemo(() => createCachedHighlighter(), []);
      ...
      <div className="w-full h-full flex flex-col">
        ...
        <div className={diffPanelStyles}>
          {hasChanges ? (
            ...
            <NoChangesView beforeCode={beforeCode} language={language} theme={theme} />
          )}
        </div>
      </div>
    </FullscreenOverlay>
...
export const DiffView = memo(({ fileHistory, setFileHistory }: DiffViewProps) => {
  ...
  if (!selectedFile || !currentDocument) {
    return (
      <div className="flex w-full h-full justify-center items-center bg-monzed-elements-background-depth-1 text-monzed-elements-textPrimary">
  ...
  try {
    ...
    return (
      <div className="flex w-full h-full justify-center items-center bg-monzed-elements-background-depth-1 text-orange-400">
