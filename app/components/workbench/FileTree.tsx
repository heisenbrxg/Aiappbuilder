...
export const FileTree = memo(
  ({
    ...
    return (
      <div className={classNames('text-sm', className, 'overflow-y-auto modern-scrollbar-enhanced')}>
...
function ContextMenuItem({ onSelect, children }: { onSelect?: () => void; children: ReactNode }) {
  return (
    <ContextMenu.Item
      onSelect={onSelect}
      className="flex items-center gap-2 px-2 py-1.5 outline-0 text-sm text-monzed-elements-textPrimary cursor-pointer ws-nowrap text-monzed-elements-item-contentDefault hover:text-monzed-elements-item-contentActive hover:bg-monzed-elements-item-backgroundActive rounded-md"
    >
...
function InlineInput({ depth, placeholder, initialValue = '', onSubmit, onCancel }: InlineInputProps) {
  ...
  return (
    <div
      className="flex items-center w-full px-2 bg-monzed-elements-background-depth-4 border border-monzed-elements-item-contentAccent py-0.5 text-monzed-elements-textPrimary"
      style={{ paddingLeft: `${6 + depth * NODE_PADDING_LEFT}px` }}
    >
      <div className="scale-120 shrink-0 i-ph:file-plus text-monzed-elements-textTertiary" />
      <input
        ref={inputRef}
        type="text"
        className="ml-2 flex-1 bg-transparent border-none outline-none py-0.5 text-sm text-monzed-elements-textPrimary placeholder:text-monzed-elements-textTertiary min-w-0"
    ...
    <>
      <ContextMenu.Root>
        <ContextMenu.Trigger>
          <div
            ...
            className={classNames('relative', {
              'bg-monzed-elements-background-depth-2 border border-dashed border-monzed-elements-item-contentAccent rounded-md':
                isDragging,
            })}
          >
            {children}
        ...
        <ContextMenu.Portal>
          <ContextMenu.Content
            style={{ zIndex: 998 }}
            className="border border-monzed-elements-borderColor rounded-md z-context-menu bg-monzed-elements-background-depth-1 dark:bg-monzed-elements-background-depth-2 data-[state=open]:animate-in animate-duration-100 data-[state=open]:fade-in-0 data-[state=open]:zoom-in-98 w-56"
          >
            <ContextMenu.Group className="p-1 border-b-px border-solid border-monzed-elements-borderColor">
            ...
            <ContextMenu.Group className="p-1 border-t-px border-solid border-monzed-elements-borderColor">
              {!isFolder ? (
                <>
    ...
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={folder.fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent text-monzed-elements-item-contentDefault hover:text-monzed-elements-item-contentActive hover:bg-monzed-elements-item-backgroundActive':
            !selected,
          'bg-monzed-elements-item-backgroundAccent text-monzed-elements-item-contentAccent': selected,
        })}
        depth={folder.depth}
      ...
      >
        <div className="flex items-center w-full">
          ...
          {isLocked && (
            <span
              className={classNames('shrink-0', 'i-ph:lock-simple scale-80 text-orange-500')}
...
function File({
  ...
  return (
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent hover:bg-monzed-elements-item-backgroundActive text-monzed-elements-item-contentDefault':
            !selected,
          'bg-monzed-elements-item-backgroundAccent text-monzed-elements-item-contentAccent': selected,
        })}
        depth={depth}
        iconElement={
          <FileIcon
            ...
            className={classNames({
              'group-hover:text-monzed-elements-item-contentActive': !selected,
            })}
          />
        }
        onClick={onClick}
          ...
          <div className="flex items-center gap-1">
            {showStats && (
              <div className="flex items-center gap-1 text-xs">
                ...
                {deletions > 0 && <span className="text-orange-500">-{deletions}</span>}
              </div>
            )}
            {locked && (
              <span
...
interface ButtonProps {
  depth: number;
  /**
   * Icon to render. If provided, this takes precedence over iconClasses.
   */
  iconElement?: ReactNode;
  /**
   * Fallback Phosphor-Uni class names for icon rendering when iconElement is not provided.
   */
  iconClasses?: string;
  children: ReactNode;
  className?: string;
...
function NodeButton({ depth, iconClasses, iconElement, onClick, className, children }: ButtonProps) {
  return (
    <button
      className={classNames(
    ...
    >
      {iconElement ? (
        <span className="scale-120 shrink-0 flex items-center justify-center">
          {iconElement}
        </span>
      ) : (
