...
export function SupabaseChatAlert({ alert, clearAlert, postMessage }: Props) {
  ...
  const executeSupabaseAction = async (sql: string) => {
    ...
    try {
      // Note: This can take 10-30 seconds if the project is paused (free tier)
      ...
      // Provide feedback to the AI about successful execution
      postMessage(
        `**<span class="i-ph:check-circle text-green-500 align-middle"></span> Supabase query executed successfully!**\n\nThe database operation completed successfully. ${(result as any).data ? `Affected ${(result as any).data.length || 0} rows.` : 'Operation completed.'} You can now proceed with the next steps.`
      );
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="max-w-chat rounded-lg border-l-2 border-l-[#098F5F] border border-monzed-elements-borderColor bg-monzed-elements-background-depth-2"
      >
        {/* Header */}
        ...
        <div className="px-4">
          {!isConnected ? (
            <div className="p-3 rounded-md bg-monzed-elements-background-depth-3">
              <span className="text-sm text-monzed-elements-textPrimary">
            ...
            <>
              <div
                className="flex items-center p-2 rounded-md bg-monzed-elements-background-depth-3 cursor-pointer"
                onClick={() => setIsCollapsed(!isCollapsed)}
              >
                <div className="i-ph:database text-monzed-elements-textPrimary mr-2"></div>
                <span className="text-sm text-monzed-elements-textPrimary flex-grow">
                  {description || 'Create table and setup auth'}
                </span>
                <div
                  className={`i-ph:caret-up text-monzed-elements-textPrimary transition-transform ${isCollapsed ? 'rotate-180' : ''}`}
                ></div>
              </div>

              {!isCollapsed && content && (
                <div className="mt-2 p-3 bg-monzed-elements-background-depth-4 rounded-md overflow-auto max-h-60 font-mono text-xs text-monzed-elements-textSecondary">
        ...
        {/* Message and Actions */}
        <div className="p-4">
          <p className="text-sm text-monzed-elements-textSecondary mb-4">{message}</p>
