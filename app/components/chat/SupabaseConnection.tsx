...
export function SupabaseConnection() {
  ...
  return (
    <div className="relative">
      <div className="flex rounded-md overflow-hidden mr-2 text-sm">
        <Button
          active
          disabled={connecting}
          onClick={() => {
          ...
          className="hover:bg-monzed-elements-item-backgroundActive flex items-center gap-2 bg-transparent"
        >
          {isConnected && supabaseConn.project && (
            <>
              <img
                ...
                alt="Supabase"
              />
              <span className="text-xs max-w-[100px] truncate text-gray-700 dark:text-gray-300">{supabaseConn.project.name}</span>
            </>
          )}
        </Button>
      </div>
          ...
          <Dialog className="max-w-[520px] p-6">
            {!isConnected ? (
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm text-monzed-elements-textSecondary mb-2">Access Token</label>
                  <input
                    type="password"
                    ...
                    className={classNames(
                      ...
                      'text-monzed-elements-textPrimary placeholder-gray-500 dark:placeholder-gray-400',
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-4 p-3 bg-[#F8F8F8] dark:bg-[#1A1A1A] rounded-lg">
                  <div>
                    <h4 className="text-sm font-medium text-monzed-elements-textPrimary">{supabaseConn.user?.email}</h4>
                    <p className="text-xs text-monzed-elements-textSecondary">Role: {supabaseConn.user?.role}</p>
                  </div>
                </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <button
                        ...
                        className="bg-transparent text-left text-sm font-medium text-monzed-elements-textPrimary flex items-center gap-2"
                      >
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="px-2 py-1 rounded-md text-xs bg-[#F0F0F0] dark:bg-[#252525] text-monzed-elements-textSecondary hover:bg-[#E5E5E5] dark:hover:bg-[#333333] flex items-center gap-1"
                    ...
                    {isProjectsExpanded && (
                      <>
                        {!supabaseConn.selectedProjectId && (
                          <div className="mb-2 p-3 bg-[#F8F8F8] dark:bg-[#1A1A1A] rounded-lg text-sm text-monzed-elements-textSecondary">
                            ...
                            {supabaseConn.stats.projects.map((project) => (
                              ...
                              >
                                <div className="flex items-center justify-between">
                                  <div>
                                    <h5 className="text-sm font-medium text-monzed-elements-textPrimary flex items-center gap-1">
                                  ...
                                  <button
                                    ...
                                    className={classNames(
                                      ...
                                      supabaseConn.selectedProjectId === project.id
                                        ? 'bg-[#3ECF8E] text-white'
                                        : 'bg-[#F0F0F0] dark:bg-[#252525] text-monzed-elements-textSecondary hover:bg-[#3ECF8E] hover:text-white',
                                    )}
                                  >
                                    {supabaseConn.selectedProjectId === project.id ? (
  ...
  return (
    <button
      className={classNames(
        ...
        {
          'bg-transparent hover:bg-monzed-elements-item-backgroundActive text-monzed-elements-textTertiary hover:text-monzed-elements-textPrimary':
            !active,
          'bg-transparent text-monzed-elements-item-contentAccent': active && !disabled,
          'bg-transparent text-alpha-gray-20 dark:text-alpha-white-20 cursor-not-allowed':
            disabled,
        },
        className,
      )}
