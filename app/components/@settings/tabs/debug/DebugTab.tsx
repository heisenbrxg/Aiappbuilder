...
import { databaseMonitor } from '~/lib/utils/databaseMonitor';
    ...
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <CollapsibleTrigger
        className={classNames(
          ...
          'hover:bg-red-50/50 dark:hover:bg-[#1a1a1a]',
      ...
      >
        <div className="flex items-center gap-3">
          <div className="i-ph:package text-monzed-elements-textSecondary w-4 h-4" />
          <span className="text-base text-monzed-elements-textPrimary">
            {title} Dependencies ({deps.length})
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-sm text-monzed-elements-textSecondary">{isOpen ? 'Hide' : 'Show'}</span>
          <div
            className={classNames(
              'i-ph:caret-down w-4 h-4 transform transition-transform duration-200 text-monzed-elements-textSecondary',
              isOpen ? 'rotate-180' : '',
            )}
          />
        </div>
          ...
          <div className="space-y-2 p-4">
            {deps.map((dep) => (
              <div key={dep.name} className="flex items-center justify-between text-sm">
                <span className="text-monzed-elements-textPrimary">{dep.name}</span>
                <span className="text-monzed-elements-textSecondary">{dep.version}</span>
              </div>
            ))}
          </div>
        </ScrollArea>
...
export default function DebugTab() {
  ...
  const exportDebugInfo = () => {
    try {
      ...
      const blob = new Blob([safeJsonStringify(debugData, 2)], { type: 'application/json' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `monzed-debug-info-${new Date().toISOString()}.json`;
  ...
  const exportAsCSV = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `monzed-debug-info-${new Date().toISOString()}.csv`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const exportAsPDF = () => {
    try {
      ...
      // Save the PDF
      doc.save(`monzed-debug-info-${new Date().toISOString()}.pdf`);
  ...
  const exportAsText = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `monzed-debug-info-${new Date().toISOString()}.txt`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  // Cleanup duplicate snapshots function
  const cleanupDuplicateSnapshots = useCallback(async () => {
    if (!user) {
      toast.error('You must be logged in to cleanup snapshots');
      return;
    }
    ...
    try {
      ...
      await supabaseDB.cleanupDuplicateSnapshots(user.id);
      toast.success('Successfully cleaned up duplicate snapshots!');
    } catch (error) {
      console.error('Failed to cleanup snapshots:', error);
      toast.error('Failed to cleanup duplicate snapshots');
    } finally {
  ...
  const checkDatabaseIntegrity = useCallback(async () => {
    if (!user) {
      toast.error('You must be logged in to check database integrity');
      return;
    }
    ...
    try {
      const stats = await databaseMonitor.getDatabaseStats(user.id);
      const integrity = await databaseMonitor.validateIntegrity(user.id);

      const message = `Database Stats:
• Total Chats: ${stats.totalChats}
• Total Snapshots: ${stats.totalSnapshots}
• Duplicate Chats: ${stats.duplicateChats}
• Orphaned Snapshots: ${stats.orphanedSnapshots}
...
${integrity.issues.length > 0 ? '\nIssues:\n• ' + integrity.issues.join('\n• ') : ''}`;
      ...
      if (integrity.isValid) {
        ...
        toast.warning('Database integrity issues found - check console for details');
      }

      console.log('Database Integrity Report:', message);
    } catch (error) {
  ...
  const cleanupDuplicateChats = useCallback(async () => {
    if (!user) {
      toast.error('You must be logged in to cleanup duplicate chats');
      return;
    }

    setLoading(prev => ({ ...prev, dbCleanup: true }));

    try {
      const result = await databaseMonitor.cleanupDuplicateChats(user.id);
      toast.success(`Cleanup completed! Removed ${result.removed} duplicates, kept ${result.kept} chats.`);
    } catch (error) {
      console.error('Failed to cleanup duplicate chats:', error);
      toast.error('Failed to cleanup duplicate chats');
    } finally {
      setLoading(prev => ({ ...prev, dbCleanup: false }));
    }
  }, [user]);
  ...
  const ExportButton = () => {
    ...
    return (
      <DialogRoot open={isOpen} onOpenChange={handleOpenChange}>
        ...
        >
          <span className="i-ph:download text-lg text-gray-500 dark:text-gray-400 group-hover:text-red-500 transition-colors" />
          Export
        </button>
              ...
              {exportFormats.map((format) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-monzed-elements-textSecondary mt-0.5">
  ...
  return (
    <div className="flex flex-col gap-6 max-w-7xl mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {/* Errors Card */}
        <div className="p-4 rounded-xl bg-white dark:bg-[#1E1E21] border border-[#E5E5E5] dark:border-[#1A1A1A] hover:border-purple-500/30 transition-all duration-200 h-[180px] flex flex-col">
          <div className="flex items-center gap-2">
            <div className="i-ph:warning-octagon text-red-500 w-4 h-4" />
            <div className="text-sm text-monzed-elements-textSecondary">Errors</div>
          </div>
          ...
          <div className="text-xs text-monzed-elements-textSecondary mt-2 flex items-center gap-1.5">
            <div
              className={classNames(
                'w-3.5 h-3.5',
        ...
        {/* Memory Usage Card */}
        <div className="p-4 rounded-xl bg-white dark:bg-[#1E1E21] border border-[#E5E5E5] dark:border-[#1A1A1A] hover:border-red-500/30 transition-all duration-200 h-[180px] flex flex-col">
          <div className="flex items-center gap-2">
            <div className="i-ph:cpu text-red-500 w-4 h-4" />
            <div className="text-sm text-monzed-elements-textSecondary">Memory Usage</div>
          </div>
          <div className="flex items-center gap-2 mt-2">
            <span
              className={classNames(
          ...
          />
          <div className="text-xs text-monzed-elements-textSecondary mt-2 flex items-center gap-1.5">
            <div className="i-ph:info w-3.5 h-3.5 text-red-500" />
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-[#1E1E21] border border-[#E5E5E5] dark:border-[#1A1A1A] hover:border-red-500/30 transition-all duration-200 h-[180px] flex flex-col">
          <div className="flex items-center gap-2">
            <div className="i-ph:timer text-red-500 w-4 h-4" />
            <div className="text-sm text-monzed-elements-textSecondary">Page Load Time</div>
          </div>
          <div className="flex items-center gap-2 mt-2">
            <span
              className={classNames(
          ...
          <div className="text-xs text-monzed-elements-textSecondary mt-2 flex items-center gap-1.5">
            <div className="i-ph:code w-3.5 h-3.5 text-red-500" />
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-[#1E1E21] border border-[#E5E5E5] dark:border-[#1A1A1A] hover:border-red-500/30 transition-all duration-200 h-[180px] flex flex-col">
          <div className="flex items-center gap-2">
            <div className="i-ph:wifi-high text-red-500 w-4 h-4" />
            <div className="text-sm text-monzed-elements-textSecondary">Network Speed</div>
          </div>
          <div className="flex items-center gap-2 mt-2">
            <span
              className={classNames(
          ...
          <div className="text-xs text-monzed-elements-textSecondary mt-2 flex items-center gap-1.5">
            <div className="i-ph:activity w-3.5 h-3.5 text-red-500" />
        ...
        <div className="md:col-span-4 p-6 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A] hover:border-purple-500/30 transition-all duration-200 h-[260px] flex flex-col">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="i-ph:robot text-red-500 w-5 h-5" />
              <div>
                <div className="text-base font-medium text-monzed-elements-textPrimary">Ollama Service</div>
                <div className="text-xs text-monzed-elements-textSecondary mt-0.5">{status.message}</div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 px-2.5 py-1 rounded-full bg-monzed-elements-background-depth-3">
                <div
              ...
              <div className="text-[10px] text-monzed-elements-textTertiary flex items-center gap-1.5">
          ...
          <div className="mt-6 flex-1 min-h-0 flex flex-col">
            {status.status === 'Running' && ollamaStatus.models && ollamaStatus.models.length > 0 ? (
              <>
                <div className="text-xs font-medium text-monzed-elements-textSecondary flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <div className="i-ph:cube-duotone w-4 h-4 text-red-500" />
                ...
                <div className="overflow-y-auto flex-1 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700 scrollbar-track-transparent hover:scrollbar-thumb-gray-400 dark:hover:scrollbar-thumb-gray-600">
                  <div className="grid grid-cols-2 gap-3 pr-2">
                    {ollamaStatus.models.map((model) => (
                      <div
                        key={model.name}
                        className="text-sm bg-monzed-elements-background-depth-3 hover:bg-monzed-elements-background-depth-4 rounded-lg px-4 py-3 flex items-center justify-between transition-colors group"
                      >
                        <div className="flex items-center gap-2 text-monzed-elements-textSecondary">
                          <div className="i-ph:cube w-4 h-4 text-red-500/70 group-hover:text-red-500 transition-colors" />
                ...
                <div className="flex flex-col items-center gap-3 max-w-[280px] text-center">
                  <div
                    className={classNames('w-12 h-12', {
                      ...
                      'i-ph:cube-duotone text-red-500/80': status.status === 'Running',
                    })}
                  />
                  <span className="text-sm text-monzed-elements-textSecondary">{status.message}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <>
            <button
              ...
              className={classNames(
                ...
                'text-monzed-elements-textPrimary',
                { 'opacity-50 cursor-not-allowed': loading.dbCheck },
              )}
            >
              {loading.dbCheck ? (
                <div className="i-ph:spinner-gap w-4 h-4 animate-spin" />
            ...
            <button
              ...
              disabled={loading.dbCleanup}
              className={classNames(
                'flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-colors',
                'bg-white dark:bg-[#1E1E21]',
                'border border-[#E5E5E5] dark:border-[#1A1A1A]',
                'hover:bg-orange-50 dark:hover:bg-[#1a1a1a]',
                'hover:border-orange-200 dark:hover:border-orange-900/30',
                'text-monzed-elements-textPrimary',
                { 'opacity-50 cursor-not-allowed': loading.dbCleanup },
              )}
            >
              {loading.dbCleanup ? (
                <div className="i-ph:spinner-gap w-4 h-4 animate-spin" />
              ) : (
                <div className="i-ph:broom w-4 h-4" />
              )}
              Cleanup Duplicate Chats
            </button>

            <button
              onClick={cleanupDuplicateSnapshots}
              ...
              className={classNames(
                ...
                'hover:border-red-200 dark:hover:border-red-900/30',
                'text-monzed-elements-textPrimary',
                { 'opacity-50 cursor-not-allowed': loading.cleanup },
              )}
            >
              {loading.cleanup ? (
                <div className="i-ph:spinner-gap w-4 h-4 animate-spin" />
      ...
      >
        <CollapsibleTrigger className="w-full">
          <div className="flex items-center justify-between p-6 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A]">
            <div className="flex items-center gap-3">
              <div className="i-ph:cpu text-red-500 w-5 h-5" />
              <h3 className="text-base font-medium text-monzed-elements-textPrimary">System Information</h3>
            </div>
            <div
              className={classNames(
        ...
        <CollapsibleContent>
          <div className="p-6 mt-2 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A]">
            {systemInfo ? (
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:desktop text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">OS: </span>
                    <span className="text-monzed-elements-textPrimary">{systemInfo.os}</span>
                  </div>
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:device-mobile text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">Platform: </span>
                    <span className="text-monzed-elements-textPrimary">{systemInfo.platform}</span>
                  </div>
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:circuitry text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">Architecture: </span>
                    <span className="text-monzed-elements-textPrimary">{systemInfo.arch}</span>
                  </div>
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:cpu text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">CPU Cores: </span>
                    <span className="text-monzed-elements-textPrimary">{systemInfo.cpus}</span>
                  </div>
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:graph text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">Node Version: </span>
                    <span className="text-monzed-elements-textPrimary">{systemInfo.node}</span>
                  </div>
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:wifi-high text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">Network Type: </span>
                    <span className="text-monzed-elements-textPrimary">
                      {systemInfo.network.type} ({systemInfo.network.effectiveType})
                    </span>
                  </div>
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:gauge text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">Network Speed: </span>
                    <span className="text-monzed-elements-textPrimary">
                  ...
                  {systemInfo.battery && (
                    <div className="text-sm flex items-center gap-2">
                      <div className="i-ph:battery-charging text-monzed-elements-textSecondary w-4 h-4" />
                      <span className="text-monzed-elements-textSecondary">Battery: </span>
                      <span className="text-monzed-elements-textPrimary">
                  ...
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:hard-drive text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">Storage: </span>
                    <span className="text-monzed-elements-textPrimary">
                ...
                <div className="space-y-2">
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:database text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">Memory Usage: </span>
                    <span className="text-monzed-elements-textPrimary">
                      {systemInfo.memory.used} / {systemInfo.memory.total} ({systemInfo.memory.percentage}%)
                    </span>
                  </div>
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:browser text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">Browser: </span>
                    <span className="text-monzed-elements-textPrimary">
                      {systemInfo.browser.name} {systemInfo.browser.version}
                    </span>
                  </div>
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:monitor text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">Screen: </span>
                    <span className="text-monzed-elements-textPrimary">
                      {systemInfo.screen.width}x{systemInfo.screen.height} ({systemInfo.screen.pixelRatio}x)
                    </span>
                  </div>
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:clock text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">Timezone: </span>
                    <span className="text-monzed-elements-textPrimary">{systemInfo.time.timezone}</span>
                  </div>
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:translate text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">Language: </span>
                    <span className="text-monzed-elements-textPrimary">{systemInfo.browser.language}</span>
                  </div>
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:chart-pie text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">JS Heap: </span>
                    <span className="text-monzed-elements-textPrimary">
                  ...
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:timer text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">Page Load: </span>
                    <span className="text-monzed-elements-textPrimary">
                      {(systemInfo.performance.timing.loadTime / 1000).toFixed(2)}s
                    </span>
                  </div>
                  <div className="text-sm flex items-center gap-2">
                    <div className="i-ph:code text-monzed-elements-textSecondary w-4 h-4" />
                    <span className="text-monzed-elements-textSecondary">DOM Ready: </span>
                    <span className="text-monzed-elements-textPrimary">
              ...
              <div className="text-sm text-monzed-elements-textSecondary">Loading system information...</div>
            )}
          </div>
        </CollapsibleContent>
      </Collapsible>
      ...
      >
        <CollapsibleTrigger className="w-full">
          <div className="flex items-center justify-between p-6 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A]">
            <div className="flex items-center gap-3">
              <div className="i-ph:chart-line text-red-500 w-5 h-5" />
              <h3 className="text-base font-medium text-monzed-elements-textPrimary">Performance Metrics</h3>
            </div>
            <div
              className={classNames(
                'i-ph:caret-down w-4 h-4 transform transition-transform duration-200',
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <div className="text-sm">
                    <span className="text-monzed-elements-textSecondary">Page Load Time: </span>
                    <span className="text-monzed-elements-textPrimary">
                      {(systemInfo.performance.timing.loadTime / 1000).toFixed(2)}s
                    </span>
                  </div>
                  <div className="text-sm">
                    <span className="text-monzed-elements-textSecondary">DOM Ready Time: </span>
                    <span className="text-monzed-elements-textPrimary">
                      {(systemInfo.performance.timing.domReadyTime / 1000).toFixed(2)}s
                    </span>
                  </div>
                  <div className="text-sm">
                    <span className="text-monzed-elements-textSecondary">Request Time: </span>
                    <span className="text-monzed-elements-textPrimary">
                      {(systemInfo.performance.timing.requestTime / 1000).toFixed(2)}s
                    </span>
                  </div>
                  <div className="text-sm">
                    <span className="text-monzed-elements-textSecondary">Redirect Time: </span>
                    <span className="text-monzed-elements-textPrimary">
                ...
                <div className="space-y-2">
                  <div className="text-sm">
                    <span className="text-monzed-elements-textSecondary">JS Heap Usage: </span>
                    <span className="text-monzed-elements-textPrimary">
                      {(systemInfo.performance.memory.usedJSHeapSize / (1024 * 1024)).toFixed(1)}MB /{' '}
                      {(systemInfo.performance.memory.totalJSHeapSize / (1024 * 1024)).toFixed(1)}MB
                    </span>
                  ...
                  <div className="text-sm">
                    <span className="text-monzed-elements-textSecondary">Heap Utilization: </span>
                    <span className="text-monzed-elements-textPrimary">
                      {systemInfo.performance.memory.usagePercentage.toFixed(1)}%
                    </span>
                  </div>
                  <div className="text-sm">
                    <span className="text-monzed-elements-textSecondary">Navigation Type: </span>
                    <span className="text-monzed-elements-textPrimary">
                  ...
                  <div className="text-sm">
                    <span className="text-monzed-elements-textSecondary">Redirects: </span>
                    <span className="text-monzed-elements-textPrimary">
        ...
        <CollapsibleTrigger className="w-full">
          <div className="flex items-center justify-between p-6 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A]">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-base font-medium text-monzed-elements-textPrimary">WebApp Information</h3>
        ...
        <CollapsibleContent>
          <div className="p-6 mt-2 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A]">
            {loading.webAppInfo ? (
              ...
              <div className="flex flex-col items-center justify-center p-8 text-monzed-elements-textSecondary">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <h3 className="mb-4 text-base font-medium text-monzed-elements-textPrimary">Basic Information</h3>
                  <div className="space-y-3">
                    <div className="text-sm flex items-center gap-2">
                      <div className="i-ph:app-window text-monzed-elements-textSecondary w-4 h-4" />
                      <span className="text-monzed-elements-textSecondary">Name:</span>
                      <span className="text-monzed-elements-textPrimary">{webAppInfo.name}</span>
                    </div>
                    <div className="text-sm flex items-center gap-2">
                      <div className="i-ph:tag text-monzed-elements-textSecondary w-4 h-4" />
                      <span className="text-monzed-elements-textSecondary">Version:</span>
                      <span className="text-monzed-elements-textPrimary">{webAppInfo.version}</span>
                    </div>
                    <div className="text-sm flex items-center gap-2">
                      <div className="i-ph:certificate text-monzed-elements-textSecondary w-4 h-4" />
                      <span className="text-monzed-elements-textSecondary">License:</span>
                      <span className="text-monzed-elements-textPrimary">{webAppInfo.license}</span>
                    </div>
                    <div className="text-sm flex items-center gap-2">
                      <div className="i-ph:cloud text-monzed-elements-textSecondary w-4 h-4" />
                      <span className="text-monzed-elements-textSecondary">Environment:</span>
                      <span className="text-monzed-elements-textPrimary">{webAppInfo.environment}</span>
                    </div>
                    <div className="text-sm flex items-center gap-2">
                      <div className="i-ph:graph text-monzed-elements-textSecondary w-4 h-4" />
                      <span className="text-monzed-elements-textSecondary">Node Version:</span>
                      <span className="text-monzed-elements-textPrimary">{webAppInfo.runtimeInfo.nodeVersion}</span>
                    </div>
                  </div>
                </div>

                <div>
                  <h3 className="mb-4 text-base font-medium text-monzed-elements-textPrimary">Git Information</h3>
                  <div className="space-y-3">
                    <div className="text-sm flex items-center gap-2">
                      <div className="i-ph:git-branch text-monzed-elements-textSecondary w-4 h-4" />
                      <span className="text-monzed-elements-textSecondary">Branch:</span>
                      <span className="text-monzed-elements-textPrimary">{webAppInfo.gitInfo.local.branch}</span>
                    </div>
                    <div className="text-sm flex items-center gap-2">
                      <div className="i-ph:git-commit text-monzed-elements-textSecondary w-4 h-4" />
                      <span className="text-monzed-elements-textSecondary">Commit:</span>
                      <span className="text-monzed-elements-textPrimary">{webAppInfo.gitInfo.local.commitHash}</span>
                    </div>
                    <div className="text-sm flex items-center gap-2">
                      <div className="i-ph:user text-monzed-elements-textSecondary w-4 h-4" />
                      <span className="text-monzed-elements-textSecondary">Author:</span>
                      <span className="text-monzed-elements-textPrimary">{webAppInfo.gitInfo.local.author}</span>
                    </div>
                    <div className="text-sm flex items-center gap-2">
                      <div className="i-ph:clock text-monzed-elements-textSecondary w-4 h-4" />
                      <span className="text-monzed-elements-textSecondary">Commit Time:</span>
                      <span className="text-monzed-elements-textPrimary">{webAppInfo.gitInfo.local.commitTime}</span>
                    </div>
                    ...
                    {webAppInfo.gitInfo.github && (
                      <>
                        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-800">
                          <div className="text-sm flex items-center gap-2">
                            <div className="i-ph:git-fork text-monzed-elements-textSecondary w-4 h-4" />
                            <span className="text-monzed-elements-textSecondary">Repository:</span>
                            <span className="text-monzed-elements-textPrimary">
                        ...
                        {webAppInfo.gitInfo.github.upstream && (
                          <div className="mt-2">
                            <div className="text-sm flex items-center gap-2">
                              <div className="i-ph:git-fork text-monzed-elements-textSecondary w-4 h-4" />
                              <span className="text-monzed-elements-textSecondary">Upstream:</span>
                              <span className="text-monzed-elements-textPrimary">
            ...
            {webAppInfo && (
              <div className="mt-6">
                <h3 className="mb-4 text-base font-medium text-monzed-elements-textPrimary">Dependencies</h3>
        ...
        <CollapsibleTrigger className="w-full">
          <div className="flex items-center justify-between p-6 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A]">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-base font-medium text-monzed-elements-textPrimary">Error Check</h3>
              {errorLogs.length > 0 && (
          ...
          <div className="p-6 mt-2 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A]">
            <ScrollArea className="h-[300px]">
              <div className="space-y-4">
                ...
                <div className="text-sm">
                  <span className="text-monzed-elements-textSecondary">Status: </span>
                  <span className="text-monzed-elements-textPrimary">
                    {loading.errors
                      ? 'Checking...'
                      : errorLogs.length > 0
                ...
                {errorLogs.length > 0 && (
                  <div className="mt-4">
                    <div className="text-sm font-medium text-monzed-elements-textPrimary mb-2">Recent Errors:</div>
                    <div className="space-y-2">
                      {errorLogs.map((error) => (
