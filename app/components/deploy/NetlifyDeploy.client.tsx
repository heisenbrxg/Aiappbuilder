...
export function useNetlifyDeploy() {
  ...
  const handleNetlifyDeploy = async () => {
    ...
    if (!user) {
      toast.error('Please sign in to deploy your project to Netlify');
      return false;
    }
    ...
    try {
      ...
      // Add _redirects file for proper SPA routing (if not already present)
      if (!fileContents['_redirects'] && !fileContents['/_redirects']) {
        const redirectsContent = `# Netlify redirects for SPA routing
/*    /index.html   200`;
        fileContents['_redirects'] = redirectsContent;
        console.log('Added _redirects file for proper SPA routing');
      } else {
        console.log('Using existing _redirects configuration');
      }

      // Use chatId instead of artifact.id
      const existingSiteId = localStorage.getItem(`netlify-site-${activeChatId}`);
      ...
      if (data.site) {
        localStorage.setItem(`netlify-site-${activeChatId}`, data.site.id);
      }
